...
| `개선_로드맵.md` | 사용자가 직접 작성한 전략적 개선 로드맵(2026-09-18 편입) — 초안 복구·공개 범위 재설계·백엔드 테스트/CI·DB 백업 등 P0~P2 우선순위. TO-DO-LIST와 달리 완료 체크리스트가 아니라 방향 제시 문서 — 실제 착수 시 개별 항목을 TO-DO-LIST에 등록해 따로 추적한다 |
...
- **Vercel 배포는 계획이 아닙니다.** 초기 기획에는 Vercel 정적 배포가 있었으나, DB 저장(FR-08)을 위해 FastAPI 백엔드를 도입하면서 무효가 됐습니다 — Vercel 서버리스 함수는 매 요청마다 새로 뜨는 구조라 SQLite 파일을 유지할 수 없습니다. 기획 원문에 남아 있는 Vercel 언급을 근거로 정적 배포를 가정하지 마세요. **공개 배포는 2026-09-17 Fly.io로 확정했습니다**(위 "현재 상태" 참조) — Docker 이미지 하나로 프론트+백엔드+SQLite를 같은 오리진에서 서빙하는 지금 구조를 그대로 씁니다.
...
- **개발은 Docker를 쓰지 않고 로컬 2프로세스(5173 + 8000)로 합니다.** 다만 저장소 루트의 `Dockerfile`/`docker-compose.yml`은 **배포용**으로 별도 존재합니다(TO-DO 10, "단일 이미지로 가자" 확정) — 멀티스테이지 빌드로 프론트(Vite 빌드 산출물)와 백엔드를 한 이미지에 묶어, FastAPI가 `/api/*`는 API로 그 외 경로는 정적 파일 또는 `index.html`(SPA 폴백)로 같은 오리진(포트 8000)에서 서빙합니다 — 그래서 배포 환경엔 CORS 설정이 필요 없습니다. SQLite는 `./data`를 컨테이너의 `/app/data`에 바인드 마운트(로컬 compose) 또는 Fly.io 볼륨(배포, 아래 "현재 상태" 참조)으로 영속화해 컨테이너를 지워도 데이터가 남습니다. Windows에서 컨테이너 안 Vite HMR이 자주 실패하므로 프론트는 로컬 개발에서 컨테이너에 넣지 않고 로컬 Vite로 개발합니다. `backend/config.py`의 `DATABASE_URL` 기본값은 컨테이너·로컬 양쪽에서 동일하게 동작하도록 프로젝트 루트 `data/tacticore.db` 절대경로를 가리킵니다 — 참고 프로젝트 템플릿을 맞추려고 가져온 것이 아니라 이 프로젝트의 배포 경로로 의도적으로 추가된 것이니 되돌리지 마세요.
