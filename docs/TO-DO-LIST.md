...
- **너비 수정 후 확인("잘 뜨는데 선수노드를 움직이려하면 맨아래로 움직여져")**: 피치는 이제 보이는데, 드래그하면 항상 화면 맨 아래로 튀는 새 버그.
  - **버그 6 — 선수 드래그 좌표 계산이 Framer Motion의 `PanInfo.point`(page 좌표)를 client 좌표로 잘못 취급**: `PlayerNode.tsx`/`OpponentNode.tsx`/`PressingLine.tsx` 셋 다 `onPan` 핸들러에서 `info.point.x/y`를 그대로 `clientToPitch(svg, clientX, clientY)`에 넘기고 있었다. `clientToPitch`는 `svg.getScreenCTM()`을 쓰므로 **뷰포트 기준(client) 좌표**를 기대하는데, Framer Motion 소스(`node_modules/framer-motion/dist/es/events/event-info.mjs`)를 직접 확인하니 `PanInfo.point`는 실제로 `event.pageX`/`event.pageY` — **페이지 기준(스크롤 포함) 좌표**였다. 두 좌표계의 차이는 정확히 `window.scrollX/scrollY`. 데스크톱 3열 그리드에서는 피치가 스크롤 없이 바로 보여서 스크롤량이 거의 0이라 이 오차가 드러나지 않았지만, 모바일 1열 레이아웃은 피치까지 도달하려면 늘 수백 px를 스크롤해야 해서 그 스크롤량만큼 좌표가 어긋나 `clampCoord`가 값을 99.9(피치 맨 아래)로 눌러버렸다 — 이번 세션에서 새로 만든 버그가 아니라 드래그 이동 기능이 생긴 이래 있었지만 모바일 레이아웃이 나오기 전까지 드러날 조건(스크롤된 채로 드래그)이 거의 없던 잠복 버그. `DrawOverlay.tsx`(화살표 그리기)는 네이티브 `e.clientX/clientY`를 써서 애초에 이 문제가 없었다 — Framer Motion의 `onPan`을 쓰는 3곳만 해당.
    - **수정**: `lib/coords.ts`에 `pagePointToPitch(svg, pageX, pageY)`를 추가 — `clientToPitch(svg, pageX - window.scrollX, pageY - window.scrollY)`로 감싼다. `PlayerNode.tsx`·`OpponentNode.tsx`·`PressingLine.tsx`의 `onPan` 핸들러가 `clientToPitch(svg, info.point.x, info.point.y)` 대신 `pagePointToPitch(svg, info.point.x, info.point.y)`를 쓰도록 교체.
    - **검증**: `tsc -b`/`lint`(0 errors)/`vitest`(174, 회귀 없음)/`build` 통과. Playwright로 390px 뷰포트에서 피치가 보이는 지점까지 실제로 스크롤(`scrollY≈724`)한 뒤 선수 노드를 화면상 40px만 드래그 → 결과가 즉시 맨 아래로 튀지 않고 약 36px만큼 비례해서 이동하는 것을 좌표로 직접 확인(수정 전이었다면 스크롤량 724px가 그대로 오차가 되어 어떤 드래그든 즉시 맨 아래로 클램프됐을 상황).
- **미확인**: 이번 두 수정(너비·드래그 좌표) 모두 실제 iOS Safari에서 의도대로 동작하는지는 사용자가 재확인해야 한다.
