...
- **실배포 검증(2026-09-26)**: tacticraft.fly.dev에서 확인한 것 — ① 커뮤니티 게시물(내 소유)의 공유 페이지에서 리믹스 버튼이 정상적으로 안 보임(소유자 숨김 로직 확인). ② 에디터에서 "리믹스 허용" ↔ "리믹스 금지" 토글을 두 번 눌러 라벨·토스트("리믹스를 막았습니다"/"다른 사람이 리믹스할 수 있습니다")가 즉시 반영되는 것 확인, 원래 상태(허용)로 복원해둠. **다른 계정으로 실제 리믹스 버튼을 눌러보는 것(비소유자 시점)은 확인하지 않음** — 이 세션이 로그인 중인 계정이 하나뿐이라, 운영 사이트에 테스트용 두 번째 계정을 새로 만들지 않는 쪽을 택했다. 이 경로는 `test_remix.py`의 `test_successful_remix_preserves_provenance_and_copies_data` 등으로 백엔드 로직은 충분히 검증됨 — 사용자가 원하면 실제 다른 계정으로 눌러서 에디터 진입·출처 표시까지 확인해 달라.
- **남은 일**: 위 "비소유자 시점" 확인. 확인되면 `[C]`로 바꿔 `TO-DO-ARCHIVE.md`로 이동.
