...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      // pointer-events-none — 토스트가 하나도 없을 때도 이 뷰포트 자체는
      // 항상 DOM에 렌더링된다(Radix). 이게 auto였으면 z-[100]의 투명한
      // 전체 너비 스트립이 상단 헤더(로고·내비) 위를 덮어 클릭을 가로챈다
      // (advisor 리뷰로 발견 — 토스트가 하나도 없는 평소 화면에서도 재현되는
      // 버그였다). 각 토스트 자체는 아래 toastVariants의 pointer-events-auto로
      // 다시 켜지므로 실제 토스트는 여전히 클릭·스와이프된다.
