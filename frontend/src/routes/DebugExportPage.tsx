...
export function DebugExportPage() {
  ...
  /**
   * 2번째 테스트(2026-09-20, 여러 차례 개정) — 실제 프로덕션 함수
   * (lib/exportImage.ts의 captureThumbnail)를 그대로 호출한다.
   *
   * 한때 이 자리에서 html-to-image 내부 rAF 대기가 "멈추는 원인"이라고
   * 보고 rAF 없는 직접 구현으로 바꿔 배포한 적이 있는데, 그건 이 진단
   * 페이지 자체를 자동화 도구(claude-in-chrome)로 열었을 때
   * document.visibilityState가 강제로 hidden이 되어 rAF가 안 돌았던
   * 현상을 실제 버그로 오인한 것이었다 — 되돌렸다. 지금은 다시
   * html-to-image의 기본 rAF 대기를 그대로 쓰는 프로덕션 경로를
   * 검증 없이 그대로 호출만 한다. 15초 타임아웃은 안전장치일 뿐이고,
   * 결과 이미지에 피치가 실제로 보이는지는 이 페이지를 연 사람이
   * 눈으로 확인해야 한다 — 자동화 도구로는 신뢰할 수 있게 판단할 수 없다.
   */
